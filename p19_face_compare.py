...
class Config:
    def __init__(self):
        ...
        self.vector_size = 2  # should be about 90000 in practice.
...
class Tensors:
    def __init__(self, config: Config):
        ...
        with tf.device('/gpu:0'):
            ...
            self.x1 = tf.placeholder(tf.float32, [None, 224, 224, 3], 'x1')
            x1 = self.x1 / 255

            self.x2 = tf.placeholder(tf.float32, [None, 224, 224, 3], 'x2')
            x2 = self.x2 / 255


            with tf.variable_scope('resnet'):
                vector1 = self.resnet(x1)
                tf.get_variable_scope().reuse_variables()
                vector2 = self.resnet(x2)
            ...
            self.y_predict = (tf.reduce_sum(vector1*vector2, axis = 1)+1)/2.

            self.y = tf.placeholder(tf.int32, [None], 'y')
            y = tf.cast(self.y, tf.float32)

            y_predict = tf.maximum(self.y_predict, config.eps)
    ...
    def resnet(self, x):
        ...
        x = tf.layers.dense(x, cfg.vector_size, name = 'dense')
        return x
...
class Samples:
    def __init__(self, config: Config):
        self.config = config
        self.x1 = np.random.uniform(low=0, high=255, size=[self.num//config.batch_size, config.batch_size, 224, 224, 3])
        self.x2 = np.random.uniform(low=0, high=255,
                                    size=[self.num // config.batch_size, config.batch_size, 224, 224, 3])

        self.y = np.random.randint(0, 2, size = [self.num//config.batch_size, config.batch_size])
        self.index = 0

    def next_batch(self, batch_size):
        x1 = self.x1[self.index, :, :, :, :]#[batch_szie, 224, 224, 3]
        x2 = self.x2[self.index, :, :, :, :]  # [batch_szie, 224, 224, 3]
        y = self.y[self.index, :]
        self.index = (self.index + 1) % np.shape(self.x1)[0]
        return x1, x2, y
    ...
    def train(self):
        ...
        for epoch in range(cfg.epoches):
            ...
            for batch in range(batches):
                x1, x2, y = self.samples.next_batch(cfg.batch_size)
                feed_dict = {
                    self.tensors.x1: x1,
                    self.tensors.x2: x2,
                    self.tensors.y: y,
