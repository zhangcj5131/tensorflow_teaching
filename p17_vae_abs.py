...
class Tensors:
    def __init__(self, config: Config):
        ...
        with tf.device('/gpu:0'):
            ...
            loss = tf.reduce_mean(tf.abs(x2 - x))
    ...
    def normalize(self,vector):
        ...
        with tf.control_dependencies([mean_assign, square_assign]):
            self.vector = vector
            return tf.identity(self.vector)
...
class Mnist:
    ...
    def predict(self, epoch):
        cfg =  self.config
        mean, square = self.session.run([self.tensors.mean, self.tensors.square])
        print('mean=', mean, 'square=', square)
        std = np.sqrt(square - mean**2)
        std = np.maximum(std, cfg.eps)
        vector = np.random.normal(loc = mean, scale=std, size = [cfg.batch_size, cfg.vector_size])
