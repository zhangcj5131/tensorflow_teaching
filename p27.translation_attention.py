...
class Sub_tensors:
    ...
    def encode(self):
        ...
        z_list = []#step1, -1, num_units
        for i in range(config.num_step1):
            xi = x[:, i]
            zi, state = cell(xi, state)
            z_list.append(zi)

        attention = self.get_attention(z_list)
        return state, attention #((c,h), (c,h))

    #z:#step1, -1, num_units
    def get_attention(self, z):
        config = self.config
        z = tf.transpose(z, [1, 0, 2])#-1, step1, units
        t = z
        t = tf.reshape(t, [-1, config.num_step1 * config.num_units])
        t = tf.layers.dense(t, config.num_step1*config.num_step2, name = 'attention')#-1, step1*step2
        t = tf.reshape(t, [-1, config.num_step2, config.num_step1, 1])
        t = tf.nn.softmax(t, axis = 2) #-1, step2, step1, 1
        z = tf.reshape(z, [-1, 1, config.num_step1, config.num_units])
        r = z * t # -1, step2, step1, units
        attention = tf.reduce_mean(r, axis = 2)#-1, stpe2, units
        return attention


    #attention: -1, step2, units
    def decode(self, state, attention):
        config = self.config
        batch_size = tf.shape(self.x)[0]
